...
<head>
    ...
    <title>{{ __('mail.payment_failed_subject', ['app' => config('app.name')]) }}</title>
</head>
                    ...
                    <tr>
                        <td style="padding:32px;">
                            ...
                            <p style="font-size:0.95rem;line-height:1.6;margin:0 0 16px;">
                                No pudimos procesar el pago de <strong>${{ number_format((float) $invoice->amount_usd, 2) }} {{ $invoice->currency }}</strong> de la factura <strong>{{ $invoice->invoice_number }}</strong> para <strong>{{ $tenant->company_display_name ?: $tenant->name }}</strong>.
                            </p>
                            <p style="font-size:0.95rem;line-height:1.6;margin:0 0 24px;">
                                Revisa tu método de pago e intenta de nuevo para evitar que se interrumpa el servicio.
                            </p>
                            <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
                                <tr>
                                    <td style="background:#12b5cb;border-radius:12px;">
                                        <a href="{{ $loginUrl }}" style="display:inline-block;padding:13px 26px;color:#04222f;font-weight:800;text-decoration:none;font-size:0.95rem;">Reintentar pago</a>
                                    </td>
                                </tr>
                            </table>
